  ...
  try {
    ...
    if (!R_Line || !R_Model || !productOrderNo || !ST_Status) {
      return NextResponse.json(
        { success: false, message: 'Missing required fields: R_Line, R_Model, productOrderNo, or ST_Status' },
        { status: 400 }
      );
    }
    ...
    await pool.request()
      ...
      .input('Log_User', sql.NVarChar, Log_User)        // ✅ Accept null
      .input('Log_UserID', sql.NVarChar, Log_UserID)    // ✅ Accept null
      .query(`
        INSERT INTO REFLOW_Log (Log_Line, Log_Model, Log_ProOrder, Log_Status, Log_User, Log_UserID, Datetime)
        VALUES (@Log_Line, @Log_Model, @Log_ProOrder, @Log_Status, @Log_User, @Log_UserID, GETDATE())
      `);
